...
from alignment.utils import test_nets, train_nets, save_checkpoint
from alignment.utils import transpose_list, condense_values

@train_nets
def train(nets, optimizers, dataset, **parameters):
    """
    direct training loop for multiple networks

    This is effectively the main supervised training pipeline, 
    optionally measuring alignment or weight changes as requested.
    """
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch_idx, batch in enumerate(ep_loop):
            ...
            if measure_alignment and (batch_idx % freq == 0):
                align_batch = []
                for net in nets:
                    # single-metric usage or multi if needed:
...
def test(nets, dataset, **parameters):
    ...
    Similar to train(), but we do not do backward passes or optimizer steps.
    We can measure alignment or other metrics if alignment=True in parameters.
    """
    wandb_run = parameters.get("run", None)
    measure_alignment = parameters.get("alignment", False)
    methods = parameters.get("methods", ["RQ"])
