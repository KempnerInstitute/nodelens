...
class TransformerWrapperEnhanced(BaseModelWrapper):
    ...
    def get_qkv_activations(
        ...
        for proj_type in ['query', 'key', 'value']:
            ...
            matching = [name for name in self.qkv_layers.get(proj_type, []) 
    ...
    def compute_per_head_representations(
        self,
    ...
    def _discover_layers(self) -> List[str]:
        ...
        try:
            ...
            trackable_layers = detect_trackable_layers(
                self._model,
                min_neurons=1,
                roles=['linear_general', 'ffn_expansion', 'ffn_contraction', 
            ...
            for name, module in self._model.named_modules():
                ...
                if isinstance(module, nn.Linear):
                    ...
                    if any(pattern in name for pattern in [
                        ...
                        'mlp', 'ffn', 'fc1', 'fc2', 'up_proj', 'down_proj', 'gate_proj'
                    ]):
    ...
    def _discover_ffn_layers(self) -> Dict[str, List[str]]:
        ...
        try:
            ...
            logger.info(f"FFN layers discovered generically: {len(ffn['expansion'])} expansion, "
                       f"{len(ffn['contraction'])} contraction")
            
            # Backward compatibility: also use old names
            self.ffn_legacy_names = {
                'up_proj': ffn['expansion'],
                'down_proj': ffn['contraction'],
                'gate_proj': ffn['gate']
            }
