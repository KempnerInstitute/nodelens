...
class ModelWrapper(BaseModelWrapper):
    ...
    def compute_forward_with_dropout(self, inputs: torch.Tensor, dropout_indices: Dict[str, torch.Tensor], mode: str = "structured") -> torch.Tensor:
        """
...
class AlignmentNetwork(ModelWrapper):
    ...
    def __init__(
        ...
        **config: Any,
        ...
        super().__init__(
            model=model, tracked_layers=tracked_layers, preprocessing_mode=preprocessing_mode, store_initial_weights=store_initial_weights, **config
        )
    ...
    def get_layer_inputs(self, images: torch.Tensor, precomputed: bool = True) -> List[torch.Tensor]:
    ...
    def _preprocess_inputs(self, raw_inputs: List[torch.Tensor], compress_convolutional: bool = True) -> List[torch.Tensor]:
            ...
            if inp.ndim == 4 and compress_convolutional:
                if self.cnn_mode == "unfold":
                    ...
                    if isinstance(module, nn.Conv2d):
                        unfolded = F.unfold(inp, kernel_size=module.kernel_size, stride=module.stride, padding=module.padding)
    ...
    def forward_targeted_dropout(
        self, images: torch.Tensor, dropout_indices: List[torch.Tensor], target_layers: List[int]
