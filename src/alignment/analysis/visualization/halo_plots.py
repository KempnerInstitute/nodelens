...
def plot_halo_redundancy_by_depth(
    ...
    # Plot bars
    bars1 = ax.bar(x - width, halo_halo_means, width, label='Within-Halo', 
                   color='#e74c3c', alpha=0.8)
    bars2 = ax.bar(x, non_halo_means, width, label='Within-Non-Halo', 
                   color='#3498db', alpha=0.8)
    bars3 = ax.bar(x + width, cross_group_means, width, label='Cross-Group', 
                   color='#2ecc71', alpha=0.8)
    
    # Add error bars if provided
    if halo_halo_stds is not None:
        ax.errorbar(x - width, halo_halo_means, yerr=halo_halo_stds, 
                   fmt='none', color='darkred', capsize=3)
    if non_halo_stds is not None:
        ax.errorbar(x, non_halo_means, yerr=non_halo_stds,
                   fmt='none', color='darkblue', capsize=3)
    if cross_group_stds is not None:
        ax.errorbar(x + width, cross_group_means, yerr=cross_group_stds,
                   fmt='none', color='darkgreen', capsize=3)
    ...
    ax2.plot(layers, halo_halo_means, 'o-', color='#e74c3c', 
             label='Within-Halo', linewidth=2, markersize=8)
    ax2.plot(layers, non_halo_means, 's-', color='#3498db',
             label='Within-Non-Halo', linewidth=2, markersize=8)
    ax2.plot(layers, cross_group_means, '^-', color='#2ecc71',
             label='Cross-Group', linewidth=2, markersize=8)
    
    # Add fill between for confidence intervals
    if halo_halo_stds is not None:
        hh_arr = np.array(halo_halo_means)
        hh_std = np.array(halo_halo_stds)
        ax2.fill_between(layers, hh_arr - hh_std, hh_arr + hh_std, 
                        color='#e74c3c', alpha=0.2)
    if non_halo_stds is not None:
        nh_arr = np.array(non_halo_means)
        nh_std = np.array(non_halo_stds)
        ax2.fill_between(layers, nh_arr - nh_std, nh_arr + nh_std,
                        color='#3498db', alpha=0.2)
    if cross_group_stds is not None:
        cg_arr = np.array(cross_group_means)
        cg_std = np.array(cross_group_stds)
        ax2.fill_between(layers, cg_arr - cg_std, cg_arr + cg_std,
...
def plot_halo_redundancy_comprehensive(
    ...
    ax = axes[0, 0]
    bins = np.linspace(0, max(max(all_halo_halo or [1]), max(all_non_halo or [1]), 
                              max(all_cross or [1])) * 1.1, 50)
    
    if all_halo_halo:
        ax.hist(all_halo_halo, bins=bins, alpha=0.5, density=True,
                label=f'Within-Halo (μ={np.mean(all_halo_halo):.3f})', color='#e74c3c')
    if all_non_halo:
        ax.hist(all_non_halo, bins=bins, alpha=0.5, density=True,
                label=f'Within-Non-Halo (μ={np.mean(all_non_halo):.3f})', color='#3498db')
    if all_cross:
        ax.hist(all_cross, bins=bins, alpha=0.5, density=True,
                label=f'Cross-Group (μ={np.mean(all_cross):.3f})', color='#2ecc71')
...
def plot_halo_redundancy_heatmap(
    ...
    # Add colorbar
    cbar = ax.figure.colorbar(im, ax=ax, label='Mean Redundancy (MI)')
