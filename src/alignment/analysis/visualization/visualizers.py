...
class MetricVisualizer:
    ...
    def plot_layer_comparison(
        ...
        for i, (bar, val) in enumerate(zip(bars, values)):
            ...
            ax.text(bar.get_x() + bar.get_width() / 2.0, height, f"{val:.3f}", ha="center", va="bottom", fontsize=9)
    ...
    def plot_metric_heatmap(
        ...
        if HAS_SEABORN:
            sns.heatmap(data, ax=ax, cmap=cmap, center=0, annot=annotate, fmt=fmt, cbar_kws={"label": "Value"}, linewidths=0.5)
        else:
            ...
            if annotate:
                for i in range(len(data.index)):
                    for j in range(len(data.columns)):
                        text = ax.text(j, i, format(data.iloc[i, j], fmt), ha="center", va="center", color="black")
...
class LayerVisualizer:
    ...
    def plot_layer_evolution(
        self, layer_data: Dict[str, Dict[int, float]], title: str = "Layer Metric Evolution", save_path: Optional[Union[str, Path]] = None
    ) -> Figure:
        """
    ...
    def plot_layer_correlation(
        self, correlation_matrix: pd.DataFrame, title: str = "Layer Correlation Matrix", save_path: Optional[Union[str, Path]] = None
    ) -> Figure:
        """
...
class ComparisonVisualizer:
    ...
    def plot_experiment_comparison(
        ...
        for idx, (metric, ax) in enumerate(zip(metric_columns, axes)):
            values = [data[data[experiment_column] == exp][metric].mean() for exp in experiments]
    ...
    def plot_metric_radar(
        self, data: Dict[str, Dict[str, float]], title: str = "Metric Radar Chart", save_path: Optional[Union[str, Path]] = None
    ) -> Figure:
        """
