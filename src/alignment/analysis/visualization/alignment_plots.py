...
class AlignmentVisualizer:
    ...
    def plot_layer_scores(
        self, scores: Dict[str, torch.Tensor], metric_name: str, save_path: Optional[str] = None, show_statistics: bool = True
    ) -> plt.Figure:
        """
    ...
    def plot_score_heatmap(
        self, scores: Dict[str, Dict[str, torch.Tensor]], save_path: Optional[str] = None, vmin: Optional[float] = None, vmax: Optional[float] = None
    ) -> plt.Figure:
        """
        ...
        metrics = sorted(set(metric for layer_scores in scores.values() for metric in layer_scores.keys()))
    ...
    def plot_pruning_analysis(
        self, pruning_results: Dict[float, Dict[str, float]], metrics: List[str] = ["accuracy", "loss"], save_path: Optional[str] = None
    ) -> plt.Figure:
        """
        ...
        Returns:
            Matplotlib figure
        """
        fig, axes = plt.subplots(1, len(metrics), figsize=(len(metrics) * 5, 5))
    ...
    def plot_neuron_importance(self, layer_name: str, scores: torch.Tensor, top_k: int = 20, save_path: Optional[str] = None) -> plt.Figure:
        """
    ...
    def create_report(self, results: Dict[str, Any], output_dir: str, experiment_name: str = "alignment_analysis"):
        """
        ...
        if "layer_scores" in results:
            for metric_name, scores in results["layer_scores"].items():
                self.plot_layer_scores(scores, metric_name, save_path=plots_dir / f"{metric_name}_distribution.png")

        if "heatmap_data" in results:
            self.plot_score_heatmap(results["heatmap_data"], save_path=plots_dir / "metrics_heatmap.png")

        if "pruning_results" in results:
            self.plot_pruning_analysis(results["pruning_results"], save_path=plots_dir / "pruning_analysis.png")
