...
from .metric_plots import METRIC_COLORS, plot_multi_metric_histogram

# Import unified pruning functions
from .pruning_plots import plot_unified_pruning_comparison
...
def plot_influence_matrix(
    ...
    for i in range(len(types)):
        for j in range(len(types)):
            ax.text(j, i, f"{matrix_norm[i, j]:.2f}", ha="center", va="center", color="black", fontsize=10)
...
def plot_cascade_test(
    ...
    ax.axhline(y=np.mean(acc_drops), color="gray", linestyle="--", label=f"Mean: {np.mean(acc_drops):.2f}%")
    ax.legend()
...
def plot_centroid_evolution(
    ...
    for ax, (x_key, y_key, x_label, y_label) in zip(axes, pairs):
        for ctype in types:
            ...
            scatter = ax.scatter(x_vals, y_vals, c=depths, cmap="viridis", s=80, edgecolors=color, linewidths=2, zorder=5)

            # Add start/end markers
            if len(x_vals) > 0:
                ax.scatter(x_vals[0], y_vals[0], marker="o", s=150, facecolors="white", edgecolors=color, linewidths=3, zorder=6)
                ax.scatter(x_vals[-1], y_vals[-1], marker="s", s=150, facecolors=color, edgecolors="black", linewidths=2, zorder=6)
...
def plot_centroid_depth_profiles(
    ...
    types = ["critical", "redundant", "synergistic", "background"]
    metrics = [("log_rq", "log(RQ) - Alignment"), ("red", "Redundancy"), ("syn", "Synergy")]
    ...
    for ax, (metric_key, metric_label) in zip(axes, metrics):
        for ctype in types:
            ...
            color = CLUSTER_COLORS.get(ctype, "#999999")
            ax.plot(depths, values, "o-", color=color, label=ctype, linewidth=2, markersize=6)
...
def plot_layer_metric_summary(
    ...
    for layer in layer_names:
        try:
            data = np.column_stack([np.asarray(layer_metrics[layer].get(m, [])).flatten() for m in metric_names])
    ...
    for i, metric in enumerate(metric_names):
        ...
        ax.fill_between(x, np.array(means) - np.array(stds), np.array(means) + np.array(stds), alpha=0.2, color=color)
...
def plot_layer_metric_trends(
    ...
    for ax, metric_name in zip(axes, metrics_to_plot):
        ...
        # Plot smoothed line
        ax.plot(x, means_smooth, "o-", color=color, linewidth=2.5, markersize=6, label=f"{metric_name.upper()} (smoothed)")

        # Show confidence interval
        if show_ci:
            ax.fill_between(x, ci_low, ci_high, alpha=0.2, color=color, label=f"{int(ci_percentile)}% CI")
    ...
    if n_layers > 20:
        ...
        axes[-1].set_xticklabels([layer_names[i].split(".")[-1] for i in tick_positions], rotation=45, ha="right", fontsize=9)
    else:
        axes[-1].set_xticks(x)
        axes[-1].set_xticklabels([n.split(".")[-1] for n in layer_names], rotation=45, ha="right", fontsize=9)
