...
class ExperimentTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        if config and hasattr(config, "num_networks"):
            assert config.num_networks == self.num_networks, f"Config num_networks ({config.num_networks}) != actual ({self.num_networks})"
    ...
    def train(
        ...
        if self.num_networks > 1:
            self.history["per_network"] = {
                i: {"train_loss": [], "train_metrics": [], "val_loss": [], "val_metrics": []} for i in range(self.num_networks)
            }
        ...
        for epoch in range(self.config.epochs):
            ...
            if self.num_networks > 1 and self.config.tensorized and self.num_networks <= 8:
                ...
                train_losses, train_metrics = self._train_epoch_tensorized(train_loader, metric_fn)
            else:
                # Train each network separately
                train_losses, train_metrics = self._train_epoch_multi(train_loader, metric_fn)
            ...
            if val_loader and (epoch + 1) % self.config.eval_interval == 0:
                if self.num_networks > 1 and self.config.tensorized and self.num_networks <= 8:
                    val_losses, val_metrics = self._validate_tensorized(val_loader, metric_fn)
                else:
                    val_losses, val_metrics = self._validate_multi(val_loader, metric_fn)
            ...
            # Log progress
            self._log_epoch_progress(epoch, avg_train_loss, avg_train_metrics, avg_val_loss, avg_val_metrics, epoch_time)
    ...
    def _train_epoch_multi(
        self, train_loader: torch.utils.data.DataLoader, metric_fn: Optional[Callable] = None
    ...
    def _train_epoch_tensorized(
        ...
        avg_metrics = [self._average_metrics(metrics) if metrics else {} for metrics in all_metrics]

        return avg_losses, avg_metrics

    def _validate_multi(
        self, val_loader: torch.utils.data.DataLoader, metric_fn: Optional[Callable] = None
    ) -> Tuple[List[float], List[Dict[str, float]]]:
    ...
    def _aggregate_metrics_dict(self, metrics_list: List[Dict[str, float]]) -> Dict[str, float]:
    ...
    def _create_optimizer_for_model(self, model: nn.Module) -> optim.Optimizer:
        ...
        if self.config.optimizer.lower() == "adam":
            return optim.Adam(model.parameters(), lr=self.config.learning_rate, **optimizer_kwargs)
        elif self.config.optimizer.lower() == "sgd":
            return optim.SGD(model.parameters(), lr=self.config.learning_rate, **optimizer_kwargs)
        elif self.config.optimizer.lower() == "adamw":
            return optim.AdamW(model.parameters(), lr=self.config.learning_rate, **optimizer_kwargs)
        else:
            raise ValueError(f"Unknown optimizer: {self.config.optimizer}")

    def _create_scheduler_for_optimizer(self, optimizer: optim.Optimizer) -> Optional[optim.lr_scheduler._LRScheduler]:
        ...
        if self.config.scheduler.lower() == "cosine":
            return optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=self.config.epochs, **scheduler_kwargs)
