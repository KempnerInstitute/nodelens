...
class ParallelPruningOptimizer:
    ...
    def __init__(self, num_workers: int = 4, use_gpu: bool = True, shared_computation: bool = True):
        """
    ...
    def compare_strategies_parallel(
        self, base_model: nn.Module, strategies: List[str], amounts: List[float], data_loader, eval_fn: Callable, layers: Optional[List[str]] = None
        ...
        experiments = [(strategy, amount) for strategy in strategies for amount in amounts]
        ...
        if self.use_gpu or self.num_workers == 1:
            ...
            for strategy, amount in experiments:
                result = self._run_single_experiment(base_model, strategy, amount, eval_fn, shared_data, layers)
                results[(strategy, amount)] = result
        else:
            # Parallel on CPU
            with ThreadPoolExecutor(max_workers=self.num_workers) as executor:
                futures = {
                    executor.submit(self._run_single_experiment, base_model, strategy, amount, eval_fn, shared_data, layers): (strategy, amount)
                    for strategy, amount in experiments
                }
    ...
    def _capture_shared_data(self, model: nn.Module, data_loader, layers: Optional[List[str]]) -> Dict:
        ...
        return {"activation_data": data, "targets": targets}

    def _run_single_experiment(
        self, base_model: nn.Module, strategy: str, amount: float, eval_fn: Callable, shared_data: Optional[Dict], layers: Optional[List[str]]
    ) -> Dict:
        ...
        if shared_data and strategy in ["alignment", "composite"]:
            ...
            if strategy == "alignment":
                scorer = NodeScoringService(metrics={"rq": get_metric("rayleigh_quotient")})
            else:  # composite
                scorer = NodeScoringService(
                    metrics={"rq": get_metric("rayleigh_quotient"), "redundancy": get_metric("pairwise_redundancy_gaussian", mode="output_based")}
                )

            layer_scores = scorer.compute_layerwise_scores(data, targets)
            scores_dict = {name: layer_scores[name].composite for name in layer_scores}
        ...
        return {"strategy": strategy, "amount": amount, "accuracy": accuracy, "masks": masks}

    def _print_comparison(self, results: Dict, strategies: List[str], amounts: List[float]):
    ...
    def prune_ensemble_parallel(self, networks: List[nn.Module], strategy: str, amount: float, shared_inputs: torch.Tensor) -> List[Dict]:
        """
        ...
        for net_idx, network in enumerate(networks):
            ...
            scores = self._compute_scores_with_shared_cov(network, shared_inputs, shared_cov, strategy)

            # Prune
            masks = self._create_and_apply_masks(network, scores, amount)

            results.append({"network_idx": net_idx, "masks": masks, "strategy": strategy, "amount": amount})
    ...
    def _compute_scores_with_shared_cov(
        self, network: nn.Module, inputs: torch.Tensor, shared_cov: Optional[torch.Tensor], strategy: str
    ...
    def _create_and_apply_masks(self, network: nn.Module, scores: Dict[str, torch.Tensor], amount: float) -> Dict[str, torch.Tensor]:
