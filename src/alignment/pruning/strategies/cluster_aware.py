...
class ClusterAwarePruning(BasePruningStrategy):
    ...
    def compute_importance_scores(
        ...
        **kwargs: Any,
        ...
        metrics = self._get_metrics(module, inputs, outputs, logits, labels, n_channels, layer_name)

        # 2. Get or compute clusters
        clusters = self._get_clusters(metrics, n_channels, layer_name)

        # 3. Get or compute halo synergy
        halo_syn = self._get_halo_syn(module, outputs, next_layer_weights, next_layer_metrics, clusters, n_channels, layer_name)
        ...
        scores = (
            self.config.alpha * log_rq_norm + self.config.beta * syn_norm - self.config.gamma * red_norm + self.config.lambda_halo * halo_syn_norm
        )
        ...
        if logits is not None and labels is not None:
            ...
            for i in range(n_channels):
                ...
                for j in partners:
                    ...
                    syn_vals.append(s)
                synergy[i] = np.mean(syn_vals) if syn_vals else 0.0
...
class ClusterAwareAdaptive(ClusterAwarePruning):
    ...
    def _analyze_cluster_distribution(self) -> Dict[str, float]:
        ...
        if not self._clusters_cache:
            return {"critical": 0.25, "synergistic": 0.25, "redundant": 0.25, "background": 0.25}
    ...
    def compute_importance_scores(
        ...
        **kwargs,
        ...
        try:
            scores = super().compute_importance_scores(module, inputs=inputs, layer_name=layer_name, **kwargs)
        finally:
    ...
    def select_channels_to_prune(
        ...
        try:
            result = super().select_channels_to_prune(scores, n_prune, layer_name, protected_indices)
        finally:
