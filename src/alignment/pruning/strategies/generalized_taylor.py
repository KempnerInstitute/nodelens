    ...
    def compute_importance_scores(
        ...
        **kwargs: Any,
        ...
        if variant == "taylor":
            ...
            structural = self.config.weight_rq * rq_norm + self.config.weight_synergy * syn_norm - self.config.weight_redundancy * red_norm
            ...
            scores = self._normalize(grad_term) * self._normalize(act_term) * (self._normalize(structural) + float(self.config.structural_eps))
            ...
            if lp is not None:
                ...
                try:
                    ...
                    scores = X @ weights
                    logger.info(
                        f"Taylor-optimal weights: Taylor={weights[0]:.3f}, RQ={weights[1]:.3f}, " f"Red={weights[2]:.3f}, Syn={weights[3]:.3f}"
                    )
                except Exception:
                    scores = taylor_norm
            else:
