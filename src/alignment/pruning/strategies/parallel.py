...
class ParallelModePruning(BasePruningStrategy):
    ...
    def __init__(self, config: Optional[PruningConfig] = None, modes: List[str] = ["low", "high", "random"], base_strategy: str = "magnitude"):
        """
    ...
    def prune_parallel(
        self, module: nn.Module, inputs: Optional[torch.Tensor] = None, amount: Optional[float] = None, **kwargs
        ...
        for mode in self.modes:
            if mode == "random":
                ...
                mask = self.create_pruning_mask(importance_scores, amount=amount, pruning_mode=mode)

            masks[mode] = mask
            sparsities[mode] = (mask == 0).float().mean().item()

        return ParallelPruningResult(masks=masks, sparsities=sparsities, importance_scores=importance_scores)
    ...
    def combine_masks(self, masks: Dict[str, torch.Tensor], method: str = "intersection") -> torch.Tensor:
        """
...
class TensorizedPruning(BasePruningStrategy):
    ...
    def analyze_pruning_patterns(self, pruning_tensor: torch.Tensor) -> Dict[str, torch.Tensor]:
        """
...
class AsyncParallelPruning(BasePruningStrategy):
    ...
    def prune_modules_parallel(
        self, modules: List[nn.Module], amounts: Union[float, List[float]], modes: List[str] = ["low"], max_workers: Optional[int] = None
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [executor.submit(prune_single_module, module, amount) for module, amount in zip(modules, amounts)]
