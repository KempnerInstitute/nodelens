...
class DependencyAwarePruning:
    ...
    def _propagate_masks(
        ...
        sorted_layers = sorted(
            self.dependency_graph.graph.values(),
    ...
    def _get_input_mask(
        ...
        if layer_dep.depends_on:
            ...
            if prev_layer in propagated:
                # Use previous layer's output mask as our input mask
                return propagated[prev_layer]['output_mask'].clone()

        # Default: keep all inputs
        in_dim = self._get_input_dimension(layer_dep.module)
        return torch.ones(in_dim, dtype=torch.bool, device=next(layer_dep.module.parameters()).device)
    ...
    def _create_weight_mask(
        ...
        if isinstance(module, nn.Linear):
            ...
            weight_mask = (
                out_mask.view(-1, 1, 1, 1) &
                in_mask.view(1, -1, 1, 1)
            ).expand_as(module.weight)
    ...
    def _validate_pruning_plan(
        self,
        propagated_masks: Dict[str, Dict[str, torch.Tensor]]
    ) -> Dict[str, Any]:
        """
        ...
        for layer_name, masks in propagated_masks.items():
            ...
            if masks['weight_mask'].shape != module.weight.shape:
                errors.append(
                    f"{layer_name}: Weight mask shape {masks['weight_mask'].shape} "
                    f"doesn't match weight shape {module.weight.shape}"
                )
            ...
            if len(masks['output_mask']) != expected_out:
                errors.append(
                    f"{layer_name}: Output mask length {len(masks['output_mask'])} "
                    f"doesn't match expected {expected_out}"
                )

            # Check input mask dimension
            expected_in = self._get_input_dimension(module)
            if len(masks['input_mask']) != expected_in:
                errors.append(
                    f"{layer_name}: Input mask length {len(masks['input_mask'])} "
                    f"doesn't match expected {expected_in}"
                )
            ...
            if dep.feeds_into:
                ...
                if next_layer in propagated_masks:
                    ...
                    if len(our_out) != len(their_in):
                        errors.append(
                            f"Dimension mismatch: {layer_name}.out ({len(our_out)}) "
                            f"→ {next_layer}.in ({len(their_in)})"
                        )
                    elif not torch.equal(our_out, their_in):
                        errors.append(
                            f"Mask mismatch: {layer_name}.out_mask != {next_layer}.in_mask"
                        )
