...
class EigenvectorDropoutExperiment(BaseExperiment):
    ...
    def _compute_layer_eigendecomposition(self) -> Dict[str, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        for layer_name, act_list in layer_activations.items():
            ...
            if total_var > 0:
                ...
                logger.info(
                    f"Layer {layer_name}: {n_components}/{len(eigenvalues)} components " f"explain {self.config.n_components_ratio*100:.1f}% variance"
                )

        return layer_eigen

    def _create_eigenvector_masks(
        self, eigendecomposition: Dict[str, Tuple[torch.Tensor, torch.Tensor]], dropout_rate: float, strategy: str = "low"
    ...
    def run(self) -> Dict[str, Any]:
        ...
        results = {
            ...
            "eigenvalues": {
                layer: eigenvalues.tolist()[:20] for layer, (eigenvalues, _) in self.eigendecomposition.items()  # Store top 20 eigenvalues
            },
            "variance_explained": {},
            ...
            for strategy in ["low", "high", "random"]:
                if strategy == "random":
                    ...
                    for trial in range(self.config.num_random_trials):
                        # Create masks
                        masks = self._create_eigenvector_masks(self.eigendecomposition, dropout_rate, strategy)
