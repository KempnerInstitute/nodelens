...
class LayerIsolatedPruningExperiment(BaseExperiment):
    ...
    def _create_dropout_masks(self, layer_scores: Dict[str, torch.Tensor], dropout_rate: float) -> Dict[str, Dict[str, torch.Tensor]]:
        """
    ...
    def _train_model(self) -> Dict[str, Any]:
        ...
        trainer = create_experiment_trainer(self.model, asdict(self.config), device=self.config.device)  # Convert dataclass to dict
    ...
    def run(self) -> Dict[str, Any]:
        ...
        for dropout_rate in self.config.dropout_rates:
            ...
            results["per_layer_masks"][f"dropout_{dropout_rate}"] = {
                layer: {"total_neurons": len(mask), "active_neurons": int(mask.sum().item())} for layer, mask in masks["low"].items()
            }
