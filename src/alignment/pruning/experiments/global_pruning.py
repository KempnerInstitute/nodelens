...
class GlobalDropoutExperiment(BaseExperiment):
    ...
    def __init__(self, config: GlobalDropoutConfig):
        ...
        # Results storage
        self.dropout_results = {"dropout_rates": self.config.dropout_rates, "metrics_by_rate": {}, "layer_statistics": {}}
    ...
    def _train_model(self) -> Dict[str, Any]:
        ...
        # Train with metrics
        history = train_with_metrics(trainer, self.data_loader, val_loader=None, compute_accuracy=True)
    ...
    def run(self, models=None, dataset=None, **kwargs) -> Dict[str, Any]:
        ...
        for dropout_rate in self.config.dropout_rates:
            ...
            if dropout_masks:
                self.wrapped_model.apply_structured_dropout(dropout_masks, mode="multiplicative", permanent=False)
            ...
            # Log progress
            self.log_metrics(step=int(dropout_rate * 100), metrics=self._flatten_metrics(metrics, dropout_rate))  # Use dropout % as step
            ...
            if self.config.checkpoint_interval > 0:
                self.save_checkpoint(step=int(dropout_rate * 100), metrics=metrics)
    ...
    def _evaluate_with_dropout(self, eval_data: torch.Tensor, dropout_rate: float) -> Dict[str, Any]:
        """
    ...
    def _flatten_metrics(self, metrics: Dict[str, Any], dropout_rate: float) -> Dict[str, float]:
    ...
    def _analyze_dropout_effects(self):
        ...
        analysis = {"metric_trends": {}, "layer_sensitivity": {}, "critical_dropout_rates": {}}
        ...
        for layer_name in self.wrapped_model.tracked_layers:
            ...
            for metric_name in self.metrics.keys():
                if metric_name in analysis["metric_trends"] and layer_name in analysis["metric_trends"][metric_name]:
