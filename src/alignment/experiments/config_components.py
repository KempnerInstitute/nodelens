...
class CNNConfig:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Convert to dictionary."""
        return {"cnn_mode": self.cnn_mode, "kernel_size": self.kernel_size, "stride": self.stride, "padding": self.padding}
...
def create_standard_training_config(epochs: int = 10, learning_rate: float = 0.001, optimizer: str = "adam", **kwargs) -> TrainingConfig:
    """Create a standard training configuration."""
    return TrainingConfig(training_epochs=epochs, learning_rate=learning_rate, optimizer=optimizer, **kwargs)


def create_standard_pruning_config(
    dropout_rates: Optional[List[float]] = None, metric: str = "rayleigh_quotient", strategy: str = "low", **kwargs
) -> PruningConfig:
    """Create a standard pruning configuration."""
    return PruningConfig(dropout_rates=dropout_rates or [0.0, 0.1, 0.3, 0.5, 0.7, 0.9], pruning_metric=metric, pruning_strategy=strategy, **kwargs)


def create_quick_test_config(epochs: int = 2, dropout_rates: Optional[List[float]] = None) -> Dict[str, Any]:
    """Create a configuration for quick testing."""
    return {
        "training": create_standard_training_config(epochs=epochs),
        "pruning": create_standard_pruning_config(dropout_rates=dropout_rates or [0.0, 0.5], num_random_trials=1),
...
def unflatten_config_dict(
    ...
    if pruning_keys is None:
        pruning_keys = ["dropout_rates", "dropout_mode", "num_random_trials", "pruning_metric", "pruning_strategy", "exclude_classification_layer"]

    if evaluation_keys is None:
        evaluation_keys = ["eval_batches", "eval_frequency", "compute_alignment_during_eval", "save_predictions"]
