    ...
    def compute_importance_scores(
        ...
        def make_hook(layer_name):
            def hook(module, input, output):
                # save activations (output of MLP)
                all_activations.setdefault(layer_name, []).append(output.detach().cpu())
            return hook

        # register hooks for MLP layers
        hooks = []
        for name, module in self._get_underlying_model().named_modules():
            if "mlp" in name.lower():  # match all MLP blocks
                hooks.append(module.register_forward_hook(make_hook(name)))

        logger.info(f"Registered {len(hooks)} MLP hooks")
        ...
        logger.info(f"Captured activations from {len(aggregated_activations)} MLP layers.")
        print("ACTIVATIONS:", list(aggregated_activations.keys()))

        # ---- Continue with metric computation ----
        metric_names = self.llm_config.alignment_metrics
        self.importance_scores = {}

        for layer_name, layer_inputs in aggregated_activations.items():
            logger.info(f"Computing importance for layer: {layer_name}")

            layer_inputs = layer_inputs.to(self.llm_config.device)
            ...
            for metric_name in metric_names:
                try:
                    ...
                    logger.debug(
                        f"{layer_name} {metric_name}: mean={float(scores.mean()):.6f} std={float(scores.std()):.6f}"
                    )
                except Exception as e:
