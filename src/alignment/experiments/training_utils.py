...
def train_with_metrics(
    ...
    history = trainer.train(
        ...
        metric_fn=metric_fn if compute_accuracy else None
    )
...
def convert_training_history(
    ...
    results = {
        ...
        'final_train_accuracy': history['train_metrics'][-1].get('accuracy', 0.0) 
            if history['train_metrics'] and history['train_metrics'][-1] else 0.0,
        'training_history': history
    }
    
    if history['val_loss']:
        results['final_val_loss'] = history['val_loss'][-1]
        results['final_val_accuracy'] = history['val_metrics'][-1].get('accuracy', 0.0) \
            if history['val_metrics'] and history['val_metrics'][-1] else 0.0
    ...
    if num_networks > 1 and 'per_network' in history:
        ...
        for i in range(num_networks):
            ...
            results['per_network_results'][i] = {
                'final_train_loss': network_history['train_loss'][-1] 
                    if network_history['train_loss'] else 0.0,
                'final_train_accuracy': network_history['train_metrics'][-1].get('accuracy', 0.0)
                    if network_history['train_metrics'] and network_history['train_metrics'][-1] else 0.0
            }
