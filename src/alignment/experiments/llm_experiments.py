...
class LLMAlignmentExperiment(BaseExperiment):
    ...
    def compute_importance_scores(self, calibration_texts: Optional[List[str]] = None, num_samples: int = 1) -> Dict[str, torch.Tensor]:
        """
        ...
        if calibration_texts is None:
            calibration_texts = ["The quick brown fox jumps over the lazy dog. " * 20]
    ...
    def run(self) -> Dict[str, Any]:
        ...
        results = {"config": self.config, "importance_scores": {}, "pruning_results": {}, "evaluation": {}}

        # Compute importance scores
        calibration_config = self.config.get("importance_computation", {})
        scores = self.compute_importance_scores(num_samples=calibration_config.get("num_samples", 1))

        # Save scores summary
        for layer_name, layer_scores in scores.items():
            results["importance_scores"][layer_name] = {
                metric: {"mean": float(scores.mean()), "std": float(scores.std()), "min": float(scores.min()), "max": float(scores.max())}
        ...
        if eval_config.get("compute_perplexity", False):
            baseline_ppl = self.evaluate_perplexity(dataset=eval_config.get("dataset", "wikitext"), num_samples=eval_config.get("num_samples", 100))
            ...
            for sparsity in sparsity_levels:
                ...
                if eval_config.get("compute_perplexity", False):
                    pruned_ppl = self.evaluate_perplexity(
                        dataset=eval_config.get("dataset", "wikitext"), num_samples=eval_config.get("num_samples", 100)
                    )
