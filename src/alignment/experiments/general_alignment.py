...
class GeneralAlignmentExperiment(BaseExperiment):
    ...
    def _tensorized_pruning_batch(self, strategy_name: str, selection_mode: str, pruning_amounts: List[float]) -> Dict[str, torch.Tensor]:
        ...
        for amount_idx, amount in enumerate(pruning_amounts):
            ...
            logger.info(f"        Before fine-tuning - Avg Acc: {batch_accs_before.mean():.2f}% (std: {acc_std:.2f}, range: [{acc_min:.2f}, {acc_max:.2f}]), Avg Sparsity: {sparsities[:, amount_idx].mean():.2%}")
    ...
    def _create_tensorized_masks_optimized(self, strategy_name: str, selection_mode: str, pruning_amounts: List[float]) -> List[List[Dict[str, torch.Tensor]]]:
        """
    ...
    def _tensorized_alignment_pruning_batch(self, strategy_name: str, selection_mode: str, pruning_amounts: List[float]) -> Dict[str, torch.Tensor]:
        ...
        for amount_idx, amount in enumerate(pruning_amounts):
            ...
            logger.info(f"        Before fine-tuning - Avg Acc: {batch_accs_before.mean():.2f}%, Avg Sparsity: {sparsities[:, amount_idx].mean():.2%}")
        ...
        if self.pruning_results and "strategies" in self.pruning_results:
            ...
            for algorithm, results in algorithm_results.items():
                ...
                if len(results["before"]) > 1:
                    ...
                    fig_before.savefig(output_dir / f"pruning_{algorithm}_accuracy_before.png",
                    ...
                    fig_after.savefig(output_dir / f"pruning_{algorithm}_accuracy_after.png",
                                     dpi=self.config.plot_dpi, bbox_inches='tight')
                    plt.close(fig_after)
                else:
