...
def progressive_dropout(nets, dataset, alignment=None, **parameters):
    ...
    # --- CHANGED ---
    # We add a 'single_layer_mode' that, if True, drops only one alignment layer
    # at a time. Then we store results separately for each layer.
    """
    ...
    for layerdata in alignment:
        ...
        # each net_i_data is a list over alignment layers => each is { "RQ": ... }
        all_nets_rq = []
        ...
        stacked = torch.stack(all_nets_rq, dim=0)  # (num_nets, total_nodes)
        alignment_layers.append(stacked)

    idx_alignment = [torch.argsort(al, dim=1) for al in alignment_layers]

    num_snapshots = len(idx_alignment)  # how many alignment-layers we have data for
    ...
    for batch in tqdm(dataloader):
        ...
        for dropidx, fraction in enumerate(drop_fraction):
            ...
            if single_layer_mode:
                # --- CHANGED ---
                # We'll do a loop over each layer individually
                for layer_i in range(num_layers):
                    if layer_i >= len(idx_high):
                        break
                    # for single-layer mode, we only drop that single alignment "snapshot" index
                    drop_high_use = [idx_high[layer_i]]
                    drop_low_use  = [idx_low[layer_i]]
                    drop_rand_use = [idx_rand[layer_i]]
                    drop_layer = [layer_i]  # we pass a single layer index
                ...
                # original logic: possibly dropping multiple layers at once
                for layer_i in range(num_layers):
                    if layer_i >= len(idx_high):
                        break
                    if by_layer:
...
def progressive_dropout_experiment(exp, nets, dataset, alignment=None, train_set=False):
    ...
    # --- CHANGED ---
    # we read single_layer_mode from config if we want:
    single_layer_mode = getattr(exp.args.extra, "single_layer_mode", False)

    dropout_params = dict(
        num_drops=exp.args.extra.num_drops,
        by_layer=exp.args.extra.dropout_by_layer,
        train_set=train_set,
        single_layer_mode=single_layer_mode,  # pass to progressive_dropout
    )
