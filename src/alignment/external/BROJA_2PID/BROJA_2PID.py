    ...
    def check_feasibility(self):  # returns pair (p,d) of primal/dual infeasibility (maxima)
        ...
        for i, xyz in enumerate(self.trip_of_idx):
            ...
            dual_infeasability = max(
                dual_infeasability, -self.sol_lambda[xy_idx] - self.sol_lambda[xz_idx] - mu_yz[(y, z)] - ln(-self.sol_lambda[i]) - 1
            )
        # ^ for
...
def pid(pdf_dirty, cone_solver="ECOS", output=0, **solver_args):
    ...
    if __debug__:
        ...
        for k, v in pdf_dirty.items():
            ...
            sum_p += v
        # ^ for
        assert abs(sum_p - 1) < 1.0e-8, "broja_2pid.pid(pdf): pdf's values must sum up to 1 (tolerance of precision is 1.e-10)"
    # ^ if
    ...
    if retval != "success":
        ...
        error_msg = (
            "BROJA_2PID: Cone Programming solver failed to find (near) optimal solution. "
            "Please report the input probability density function to abdullah.makkeh@gmail.com"
        )
