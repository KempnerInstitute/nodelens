        ...
        for i in range(num_neurons):
            ...
            try:
                ...
                if torch.linalg.matrix_rank(cov_x) == cov_x.shape[0]:
                    ...
                    h_y_given_x = 0.5 * torch.log(2 * np.pi * np.e * torch.clamp(cov_y_given_x, min=1e-10))
                else:
                    h_y_given_x = 0.5 * torch.log(2 * np.pi * np.e * torch.clamp(cov_y, min=1e-10))
