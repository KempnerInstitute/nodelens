...
if TYPE_CHECKING:
    from alignment_refac1.config import DatasetConfig, ExperimentConfig # Keep ExperimentConfig here for type hints if needed elsewhere
...
class DataSet(ABC):
    ...
    def evaluate(self, model, device='cpu', num_batches=None, show_progress: bool = True):
            ...
            if hasattr(model, 'alignment_layers'):
                for i, layer in enumerate(model.alignment_layers):
                    if hasattr(layer, 'weight') and layer.weight is not None:
                        ...
                        # Ensure this block is correctly indented under the if above
        ...
        # This line should align with the `if show_progress:` block
