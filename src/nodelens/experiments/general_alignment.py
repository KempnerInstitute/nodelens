...
from nodelens.core.registry import register_experiment
...
from nodelens.pruning.pipeline import PruningPipelineOptions, run_pruning_pipeline
from nodelens.pruning.strategies import MagnitudePruning
from nodelens.services import ActivationCaptureService, MaskOperations
...
class GeneralAlignmentExperiment(BaseExperiment):
    ...
    def _train_single_network(self) -> Dict[str, Any]:
        ...
        for epoch in pbar:
            ...
            # Update progress bar
            pbar.set_postfix(loss=f"{train_loss:.3f}", acc=f"{train_acc:.1f}%", val_acc=f"{val_acc:.1f}%")
    ...
    def _train_multiple_networks(self) -> Dict[str, Any]:
        ...
        for net in self.networks:
            if self.config.optimizer.lower() == "sgd":
                opt = torch.optim.SGD(
                    net.parameters(), lr=self.config.learning_rate, momentum=self.config.momentum, weight_decay=self.config.weight_decay
                )
            else:
    ...
    def _setup_optimizer(self) -> torch.optim.Optimizer:
        ...
        if self.config.optimizer.lower() == "sgd":
            return torch.optim.SGD(
                self.model.parameters(), lr=self.config.learning_rate, momentum=self.config.momentum, weight_decay=self.config.weight_decay
            )
    ...
    def _measure_alignment(self) -> Dict[str, Dict[str, List[float]]]:
        ...
        try:
            ...
            from nodelens.dataops.processing import preprocess_layer_activations
            ...
            preprocessed_inputs_raw = preprocess_layer_activations(inputs_to_process, layer_modules, mode=self.config.cnn_mode)
            preprocessed_outputs_raw = preprocess_layer_activations(outputs_to_process, layer_modules, mode=self.config.cnn_mode)
            ...
            for layer_name in wrapped_model_to_use.tracked_layers:
                ...
                if method == "gaussian_pid_synergy_mmi":
                    ...
                    if target_outputs is None:
                        logger.warning("gaussian_pid_synergy_mmi: target_outputs not available; skipping " f"layer {layer_name}")
                        continue
                ...
                if method == "synergy_gaussian_mmi":
                    ...
                    if targets is None:
                        logger.warning("synergy_gaussian_mmi: targets not available; skipping " f"layer {layer_name}")
                        continue
                    metric_kwargs["targets"] = targets

                # Conditional metrics that need class labels
                if method in ["conditional_rayleigh_quotient", "mi_about_class", "conditional_mi", "conditional_activation_norm", "delta_rq"]:
    ...
    def _compute_redundancy_matrices(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for layer_name in wrapped_model_to_use.tracked_layers:
                ...
                activation_data.outputs.get(layer_name)
            ...
            for selection_mode in selection_modes:
                ...
                for amount in pbar_amounts:
                    ...
                    metric_based_strategies = [
                        ...
                        "alignment_minus_redundancy",
                        "class_informative_score",
                        # Gradient-based
                        "taylor_importance",
                        "taylor_saliency",
                    ]
                    ...
                    if strategy_name == "magnitude":
                        if pruning_config.global_pruning:
                            from nodelens.pruning.strategies import GlobalMagnitudePruning
                        ...
                        from nodelens.pruning.strategies import AlignmentPruning, CascadingAlignmentPruning, GlobalAlignmentPruning
                        ...
                        from nodelens.pruning.strategies import AlignmentPruning, GlobalAlignmentPruning
                        ...
                        from nodelens.pruning.strategies import CascadingAlignmentPruning

                        alignment_metric = getattr(self.config, "pruning_alignment_metric", "rayleigh_quotient")
                        pruning_config.structured = True
                        strategy = CascadingAlignmentPruning(metric=alignment_metric, direction="forward", config=pruning_config)
                    elif strategy_name == "hybrid":
                        from nodelens.pruning.strategies import HybridPruning
                        ...
                        from nodelens.pruning.strategies import FisherPruning
                    ...
                    if isinstance(supernode_metric, str) and supernode_metric:
                        try:
                            from nodelens.core.registry import get_metric
                    ...
                    if pruning_config.global_pruning and hasattr(strategy, "prune_model"):
                        ...
                        if dependency_result is not None:
                            ...
                            layer_sparsities = {layer_name: stats.get("sparsity", 0.0) for layer_name, stats in layer_details.items()}
                ...
                for name, module in model.named_modules():
                    if hasattr(module, "weight") and len(module.weight.shape) >= 2:
                        ...
                        if layer_inputs is not None:
                            try:
                                scores = self._compute_metric_importance(module, layer_inputs, strategy_name, sample_targets)
    ...
    def _evaluate_pruning_with_precomputed_scores(
        ...
        for amount_idx, amount in enumerate(pruning_amounts):
            ...
            if amount_idx % 5 == 0 or amount_idx == num_amounts - 1:
                logger.info(f"      {amount*100:.0f}% pruning - Avg Acc: {batch_accs_before.mean():.2f}%")

        # Fine-tuning if requested
        if self.config.fine_tune_after_pruning:
            logger.info("    Fine-tuning all pruning configurations")
            for amount_idx, amount in enumerate(pruning_amounts):
                for net_idx, model in enumerate(self.networks):
                    self._restore_weights_from_dict(model, original_states[net_idx])
                    self._apply_tensorized_mask(model, all_masks[net_idx][amount_idx])
            ...
            for amount_idx, amount in enumerate(pruning_amounts):
                ...
                for name, importance_scores in layer_importance.items():
                    ...
                    if strategy_name == "magnitude" and selection_mode == "random":
                        ...
                        random_tie_scores = torch.ones_like(importance_scores)
                        mask = self._create_pruning_mask_tensor(random_tie_scores, amount, "random")
                    else:
                ...
                if strategy_name == "magnitude":
                    ...
                    for name, weight in layer_weights.items():
                        if selection_mode == "random":
                            ...
                            unique_seed = base_seed + net_idx * 10000 + hash(name) % 1000 + amount_idx * 100
                            torch.manual_seed(unique_seed)
                            random_tie_scores = torch.ones_like(weight)
                            mask = self._create_pruning_mask_tensor(random_tie_scores, amount, "random")
                        else:
    ...
    def _tensorized_alignment_pruning_batch(self, strategy_name: str, selection_mode: str, pruning_amounts: List[float]) -> Dict[str, torch.Tensor]:
        ...
        for amount_idx, amount in enumerate(pruning_amounts):
            ...
            logger.debug(f"Before fine-tuning - Avg Acc: {batch_accs_before.mean():.2f}%, Avg Sparsity: {sparsities[:, amount_idx].mean():.2%}")

        # Now process fine-tuning if requested
        if self.config.fine_tune_after_pruning:
            logger.info("    Fine-tuning all alignment pruning configurations")
    ...
    def _pruning_experiments_tensorized_detailed(self) -> Dict[str, Any]:
        ...
        logger.info("Detailed tensorized pruning is currently aliased to the aggregated tensorized path")
    ...
    def _generate_visualizations(self):
        ...
        from nodelens.analysis.visualization import generate_experiment_visualizations
    ...
    def _create_pruning_strategy(self, strategy_name: str, pruning_config: PruningConfig):
        ...
        from nodelens.pruning.strategies import GlobalAlignmentPruning, GlobalMagnitudePruning
    ...
    def _pruning_experiments_single_network(self, model: nn.Module, wrapped_model: ModelWrapper, network_id: int) -> Dict[str, Any]:
        """Perform pruning experiments on one network in the multi-network fallback path."""
        logger.info(f"Using single network pruning for network {network_id}")

        original_model = self.model
        original_wrapped_model = self.wrapped_model
        try:
            self.model = model
            self.wrapped_model = wrapped_model
            results = self._pruning_experiments_single()
            results["network_id"] = network_id
            return results
        finally:
            self.model = original_model
            self.wrapped_model = original_wrapped_model
    ...
    def _pruning_experiments_tensorized_v2(self) -> Dict[str, Any]:
        ...
        for strategy_name in self.config.pruning_strategies:
            ...
            for selection_mode in selection_modes:
                ...
                use_ultra_parallel = bool(getattr(self.config, "use_ultra_parallel_eval", False))
                if use_ultra_parallel and getattr(self.config, "fine_tune_after_pruning", False):
                    logger.info("Ultra-parallel pruning evaluation is disabled when fine-tuning is requested")
                    use_ultra_parallel = False
    ...
    def _tensorized_pruning_ultra_parallel(self, strategy_name: str, selection_mode: str, pruning_amounts: List[float]) -> Dict[str, torch.Tensor]:
        ...
        for net_idx in range(num_networks):
            for amount_idx, amount in enumerate(pruning_amounts):
                masks = all_masks[net_idx][amount_idx]
                total_units = sum(int(mask.numel()) for mask in masks.values())
                pruned_units = sum(int((mask == 0).sum().item()) for mask in masks.values())
                sparsities[net_idx, amount_idx] = pruned_units / total_units if total_units else amount
        ...
        # Fine-tuning phase
        if self.config.fine_tune_after_pruning:
            logger.info("    Ultra-parallel mode reports pre-finetuning results; caller should use batch mode when fine-tuning is required")
            accuracies_after = accuracies_before.clone()
            losses_after = losses_before.clone()
        else:
            accuracies_after = accuracies_before.clone()
