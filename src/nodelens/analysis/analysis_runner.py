...
Usage:
    from nodelens.analysis import AnalysisRunner

    runner = AnalysisRunner(config)
    runner.run()

Or via CLI:
    python -m nodelens.analysis.analysis_runner --config analysis_config.yaml
"""
...
class AnalysisRunner:
    ...
    def __init__(self, config: Union[AnalysisConfig, Dict[str, Any], str, Path]):
        ...
        self.viz = UnifiedVisualizer(style=self.config.style, figsize=self.config.figsize)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run analysis and visualization from configuration.")
    parser.add_argument(
        "--config",
        type=str,
        required=True,
