...
def plot_metric_histogram(
    ...
    # Plot histogram
    n, bin_edges, patches = ax.hist(plot_values, bins=bins, color=color, alpha=0.7, edgecolor="white", linewidth=0.5)
    ...
    if highlight_percentile is not None:
        ...
        ax.axvline(threshold_plot, color="red", linestyle="--", linewidth=2, label=f"{highlight_percentile}th percentile: {threshold:.4f}")
...
def plot_metric_violin(
    ...
    ax.set_title(f"{metric_name.replace('_', ' ').title()} Distribution Across Layers", fontsize=14, fontweight="bold")
    ax.grid(True, alpha=0.3, axis="y")
...
def plot_metric_boxplot(
    ...
    data = [np.asarray(layer_metrics[layer_name]).flatten() for layer_name in layer_names]
    ...
    ax.set_xticklabels(layer_names, rotation=45, ha="right", fontsize=9)
    ax.set_ylabel(metric_name.replace("_", " ").title(), fontsize=12)
    ax.set_title(f"{metric_name.replace('_', ' ').title()} by Layer", fontsize=14, fontweight="bold")
    ax.grid(True, alpha=0.3, axis="y")
...
def plot_metric_correlation_heatmap(
    ...
    if HAS_SEABORN:
        sns.heatmap(corr, xticklabels=metric_names, yticklabels=metric_names, annot=True, fmt=".2f", cmap="RdBu_r", center=0, vmin=-1, vmax=1, ax=ax)
    else:
...
def plot_layer_metric_heatmap(
    ...
    if HAS_SEABORN:
        sns.heatmap(matrix, xticklabels=metric_names, yticklabels=layer_names, annot=True, fmt=".2f", cmap="YlOrRd", ax=ax)
    else:
