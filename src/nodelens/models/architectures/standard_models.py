...
class MLP(nn.Module):
    ...
    This implementation matches the old nodelens.models.models.MLP functionality.
    ...
    def __init__(
        ...
        activation_map = {"relu": nn.ReLU(), "tanh": nn.Tanh(), "sigmoid": nn.Sigmoid(), "identity": nn.Identity()}
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if x.dim() > 2:
            ...
            if x.size(1) != self.input_dim:
                logger.warning(f"Input dimension mismatch. Expected {self.input_dim}, " f"got {x.size(1)}. Attempting to adjust.")
...
class CNN2P2(nn.Module):
    ...
    This implementation matches the old nodelens.models.models.CNN2P2 functionality.
...
class SimpleConvNet(nn.Module):
    ...
    def __init__(
        self, in_channels: int = 3, num_classes: int = 10, hidden_channels: List[int] = [32, 64, 128], fc_hidden: int = 256, dropout_rate: float = 0.5
    ):
        super().__init__()
        ...
        for hidden_ch in hidden_channels:
            conv_layers.extend(
                [nn.Conv2d(current_channels, hidden_ch, kernel_size=3, padding=1), nn.BatchNorm2d(hidden_ch), nn.ReLU(), nn.MaxPool2d(2, 2)]
            )
        ...
        self.classifier = nn.Sequential(
            nn.Dropout(dropout_rate), nn.Linear(flattened_size, fc_hidden), nn.ReLU(), nn.Dropout(dropout_rate), nn.Linear(fc_hidden, num_classes)
        )
...
def create_model(model_name: str, dataset_name: Optional[str] = None, **kwargs) -> nn.Module:
    """
    ...
    if model_name == "mlp":
        ...
        raise ValueError(f"Unknown model name: {model_name}. " f"Available models: mlp, cnn2p2, simple_conv")
