...
Usage:
    from nodelens.configs.unified_config import load_unified_config, UnifiedConfig
...
class EvaluationConfig:
    ...
    def from_dict(cls, d: Dict[str, Any]) -> "EvaluationConfig":
        ...
        if "perplexity" in d:
            ...
            if "datasets" in ppl:
                config.perplexity_datasets = [ds["name"] if isinstance(ds, dict) else ds for ds in ppl["datasets"]]

        if "benchmarks" in d:
            config.benchmarks_enabled = True
            config.benchmark_tasks = [b["name"] if isinstance(b, dict) else b for b in d["benchmarks"]]
...
class VisualizationConfig:
    ...
    # Scatter plot pairs
    scatter_pairs: List[List[str]] = field(
        default_factory=lambda: [
