...
class BatchMetricProcessor:
    ...
    def __init__(self, device: torch.device = None, max_memory_gb: float = 8.0, show_progress: bool = True):
        """
            ...
            for batch_idx, (inputs, _) in enumerate(dataloader):
                ...
                for layer_name in model_wrapper.tracked_layers:
                    ...
                    for metric_name, metric in metrics.items():
                        scores = self._compute_metric_safe(metric, inputs=layer_inputs, weights=layer_weights, outputs=layer_outputs)
    ...
    def _check_gpu_memory(self):
        ...
        if torch.cuda.is_available():
            ...
            if memory_used > self.max_memory_gb * 0.9:
                logger.warning(f"GPU memory usage high: {memory_used:.2f}GB used, " f"{memory_reserved:.2f}GB reserved")
...
def compute_metrics_parallel(
    model_wrapper, dataloader: DataLoader, metrics: Dict[str, Any], num_workers: int = 4, devices: Optional[List[torch.device]] = None
) -> Dict[str, Dict[str, torch.Tensor]]:
    """
    ...
    def process_chunk(chunk_data, device_id):
        ...
        chunk_loader = DataLoader(chunk_data, batch_size=dataloader.batch_size, shuffle=False, num_workers=0)  # Avoid nested multiprocessing
...
def batch_mutual_information(X: torch.Tensor, Y: torch.Tensor, bins: int = 10, batch_size: int = 1000, method: str = "histogram") -> torch.Tensor:
    """
    ...
    for i in range(0, n_pairs, batch_size):
        ...
        for j in range(batch_x.shape[0]):
            mi_values[i + j] = gpu_mutual_information(batch_x[j], batch_y[j], bins=bins, method=method)
