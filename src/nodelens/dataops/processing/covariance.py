...
class CovarianceEstimator:
    ...
    def _oas_shrinkage(self, X_centered: torch.Tensor, cov_sample: torch.Tensor) -> torch.Tensor:
        ...
        if n_samples > 1:
            ...
            rho = ((1 - 2 / n_features) * trace_S2 + trace_S**2) / ((n_samples + 1 - 2 / n_features) * (trace_S2 - trace_S**2 / n_features) + 1e-8)
            shrinkage = rho.to(device=X_centered.device, dtype=X_centered.dtype)
        else:
            shrinkage = torch.ones((), device=X_centered.device, dtype=X_centered.dtype)
