...
class WandaPruning(BasePruningStrategy):
    ...
    def compute_importance_scores(
        self, module: nn.Module, inputs: Optional[torch.Tensor] = None, layer_name: Optional[str] = None, **kwargs
    ) -> torch.Tensor:
        """
        ...
        if activation_norm.shape[0] != weight.shape[1]:
            logger.warning(
                f"Activation norm shape {activation_norm.shape} doesn't match " f"weight in_features {weight.shape[1]}. Using weight magnitude only."
            )
            return weight.abs()
...
def compute_wanda_scores(
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear):
            scores[name] = strategy.compute_importance_scores(module, layer_name=name)
...
class OWLPruning(WandaPruning):
    ...
    def calibrate(
        ...
        logger.info(
            f"OWL calibration complete. Outlier ratios: "
    ...
    def compute_importance_scores(
        ...
        if layer_name and layer_name in self.activation_norms:
            ...
            if std > 1e-10:
                ...
                z_scores.abs() > self.outlier_threshold
