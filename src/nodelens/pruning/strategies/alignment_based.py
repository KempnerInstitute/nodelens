...
class AlignmentPruning(BasePruningStrategy):
    ...
    def compute_importance_scores(self, module: nn.Module, inputs: Optional[torch.Tensor] = None, **kwargs) -> torch.Tensor:
        ...
        if inputs is None:
            raise ValueError("AlignmentPruning requires inputs to compute alignment scores. " "Pass inputs to the prune() method.")
...
class GlobalAlignmentPruning(AlignmentPruning):
    ...
    Examples:
        >>> from nodelens.pruning.strategies import GlobalAlignmentPruning
        >>> from nodelens.pruning import PruningConfig
        >>>
