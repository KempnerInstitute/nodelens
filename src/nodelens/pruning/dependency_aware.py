...
class DependencyGraph:
    ...
    def _detect_skip_connections(self) -> None:
        """
        Mark residual-compatible convolution groups.

        This graph is built from module names, not a traced forward pass. We therefore
        keep the rule conservative: only convolutions that preserve channel count and
        spatial size are marked as residual-compatible candidates. The pruning code
        still validates tensor shapes before applying masks.
        """
        residual_candidates: Dict[tuple[str, int], List[str]] = {}
        ...
        for name, dep in self.graph.items():
            ...
            module = dep.module
            if not (hasattr(module, "in_channels") and hasattr(module, "out_channels")):
                continue
            if module.in_channels != module.out_channels:
                continue

            stride = getattr(module, "stride", 1)
            stride_values = stride if isinstance(stride, tuple) else (stride,)
            if any(int(value) != 1 for value in stride_values):
                continue

            parent = name.rsplit(".", 1)[0] if "." in name else ""
            key = (parent, int(module.out_channels))
            residual_candidates.setdefault(key, []).append(name)

        for names in residual_candidates.values():
            for name in names:
                peers = [candidate for candidate in names if candidate != name]
                self.graph[name].skip_connection_with = peers
