...
def _prepare_inputs_and_weights(inputs: torch.Tensor, weights: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    """Flatten inputs and weights to the two-dimensional form used by these metrics."""
    if inputs.ndim != 2:
...
def _project_neuron_outputs(inputs: torch.Tensor, weights: torch.Tensor) -> torch.Tensor:
    """Project layer inputs through neuron weights."""
    return torch.matmul(inputs, weights.T)


def _gaussian_mi_matrix_from_projected(projected: torch.Tensor) -> torch.Tensor:
    """Compute the Gaussian pairwise mutual-information matrix from neuron outputs."""
    ...
    rho_sq = torch.clamp(corr_matrix**2, 0, 0.999999)
    mi_matrix = -0.5 * torch.log(1.0 - rho_sq)
    mi_matrix.fill_diagonal_(0)
    return torch.nan_to_num(mi_matrix)
...
class AverageRedundancy(BaseMetric):
    ...
    def compute(
        ...
        inputs, weights = _prepare_inputs_and_weights(inputs, weights)
        ...
        projected = _project_neuron_outputs(inputs, weights)  # [batch_size, num_neurons]
        ...
        if num_neurons > 2048:
            ...
            mi_matrix = _gaussian_mi_matrix_from_projected(projected)
...
class LayerRedundancy(BaseMetric):
    ...
    def __init__(self, return_matrix: bool = False, max_matrix_neurons: int = 2048, **config: Any):
        """
        Initialize layer redundancy metric.

        Args:
            return_matrix: If True, return full redundancy matrix
            max_matrix_neurons: Maximum number of neurons for full matrix output
            **config: Additional configuration
        """
        super().__init__(**config)
        self.return_matrix = return_matrix
        self.max_matrix_neurons = max_matrix_neurons
    ...
    def compute(
        ...
        if self.return_matrix:
            if inputs is None or weights is None:
                raise ValueError("LayerRedundancy requires inputs and weights")

            inputs, weights = _prepare_inputs_and_weights(inputs, weights)
            batch_size = inputs.shape[0]
            num_neurons = weights.shape[0]

            if batch_size < self._avg_redundancy.min_samples:
                logger.warning(f"LayerRedundancy: Only {batch_size} samples, returning zero matrix")
                return torch.zeros(num_neurons, num_neurons, device=weights.device, dtype=weights.dtype)

            if num_neurons > self.max_matrix_neurons:
                raise ValueError(
                    "LayerRedundancy(return_matrix=True) would allocate a "
                    f"{num_neurons}x{num_neurons} matrix; set max_matrix_neurons "
                    "higher if this is intentional."
                )

            projected = _project_neuron_outputs(inputs, weights)
            if self._should_use_cpu(projected):
                projected = projected.cpu()
            return _gaussian_mi_matrix_from_projected(projected).to(weights.device)
