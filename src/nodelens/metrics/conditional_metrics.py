        ...
        **kwargs: Any,
        ...
        if batch_size != target_batch_size:
            if batch_size % target_batch_size == 0:
                ...
                logger.debug(f"Conditional RQ: Expanded targets from {target_batch_size} to {batch_size} " f"({num_patches} patches per sample)")
            else:
        ...
        **kwargs: Any,
        ...
        if batch_size != target_batch_size:
            if batch_size % target_batch_size == 0:
                ...
                num_patches = batch_size // target_batch_size
                targets = targets.repeat_interleave(num_patches)
                logger.debug(f"MI about class: Expanded targets from {target_batch_size} to {batch_size} " f"({num_patches} patches per sample)")
            else:
                # Unexpected mismatch
                logger.warning(
                    f"MI about class: Unexpected batch size mismatch (outputs: {batch_size}, " f"targets: {target_batch_size}). Returning zeros."
                )
                return torch.zeros(num_neurons, device=device)
