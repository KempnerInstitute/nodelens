    ...
    for prune_percent in dropout_percents:
        ...
        for strategy in strategies:
            ...
            for i, layer in enumerate(network.alignment_layers):
                ...
                # Get weights and compute importance scores
            ...
            for i, layer in enumerate(network.alignment_layers):
                if hasattr(layer, "weight") and layer.weight is not None:
                    # Count how many truly zero rows we have (pruned neurons)
                    weights = layer.weight.data
                    zero_rows = 0
                    for j in range(weights.shape[0]):  # Iterate over rows (neurons)
                        if torch.all(weights[j, :] == 0).item():
                            zero_rows += 1

                    zero_verification[i] = zero_rows
                    total_zeros += zero_rows

            print(f"  Verification: {total_zeros} neurons were correctly zeroed out")
