...
class TestClassConditionedRQ:
    ...
    def test_basic_class_conditioned_computation(self):
        ...
        rq_cond = metric.compute_class_conditioned(
            inputs, weights, targets,
            return_delta_rq=False
        )
    ...
    def test_delta_rq_computation(self):
        ...
        metric = RayleighQuotient()
        results = metric.compute_class_conditioned(
            inputs, weights, targets,
    ...
    def test_delta_rq_invariance_to_scaling(self):
        ...
        results1 = metric.compute_class_conditioned(
            inputs, weights, targets, return_delta_rq=True
        )
        
        # Scale weights by constant
        weights_scaled = weights * 2.0
        results2 = metric.compute_class_conditioned(
            inputs, weights_scaled, targets, return_delta_rq=True
        )
