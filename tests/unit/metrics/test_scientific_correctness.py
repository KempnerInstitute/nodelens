...
class TestRedundancyCorrectness:
    ...
    def test_orthogonal_weights_low_redundancy(self):
        ...
        assert redundancy.mean() < 0.15, \
            f"Orthogonal weights should have low redundancy, got {redundancy.mean():.4f}"
    ...
    def test_colinear_weights_high_redundancy(self):
        ...
        assert redundancy.mean() > 0.8, \
            f"Colinear weights should have high redundancy, got {redundancy.mean():.4f}"
    ...
    def test_output_based_matches_covariance_based(self):
        ...
        assert correlation > 0.95, \
            f"Output-based and covariance-based should match, correlation = {correlation:.4f}"
...
class TestDeltaRQCorrectness:
    ...
    def test_class_separated_data_high_delta_rq(self):
        ...
        assert results['delta_rq'][0] > 0.01, \
            f"Expected positive ΔRQ for separating dim, got {results['delta_rq'][0]:.4f}"
    ...
    def test_single_class_zero_delta_rq(self):
        ...
        assert torch.abs(results['delta_rq']).mean() < 0.01, \
            f"Single class should give ΔRQ ≈ 0, got {results['delta_rq'].mean():.4f}"
...
class TestMutualInformationCorrectness:
    ...
    def test_independent_variables_zero_mi(self):
        ...
        assert mi < 0.15, \
            f"Independent variables should have MI ≈ 0, got {mi:.4f}"
    ...
    def test_correlated_variables_positive_mi(self):
        ...
        # Should be positive
        assert mi > 0.5, \
            f"Correlated variables should have MI > 0.5, got {mi:.4f}"
    ...
    def test_deterministic_relationship_high_mi(self):
        ...
        # Should be high
        assert mi > 1.0, \
            f"Deterministic relationship should have high MI, got {mi:.4f}"
...
class TestRayleighQuotientCorrectness:
    ...
    def test_rq_top_eigenvector_maximum(self):
        ...
        weights = torch.stack([
            top_eigenvector,
            torch.randn(D) / D**0.5  # Random direction
        ])
        ...
        assert scores[0] > scores[1], \
            f"Top eigenvector should have highest RQ: {scores[0]:.4f} vs {scores[1]:.4f}"
...
class TestSynergyCorrectness:
    ...
    def test_identical_neurons_zero_synergy(self):
        ...
        assert torch.abs(synergy).mean() < 0.2, \
            f"Identical neurons should have near-zero synergy, got {synergy.mean():.4f}"
...
class TestScaleInvariance:
    ...
    def test_rq_scale_invariance(self):
        ...
        # Should be identical
        assert torch.allclose(scores1, scores2, rtol=1e-4), \
            "RQ should be invariant to weight scaling"
    ...
    def test_delta_rq_scale_invariance(self):
        ...
        assert torch.allclose(results1['delta_rq'], results2['delta_rq'], rtol=1e-3), \
            "ΔRQ should be invariant to scaling"
...
def run_all_validation_tests():
    ...
    test_classes = [
        TestRedundancyCorrectness,
        TestDeltaRQCorrectness,
        TestMutualInformationCorrectness,
