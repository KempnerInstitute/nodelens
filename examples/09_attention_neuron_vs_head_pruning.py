...
from alignment.services import NodeScoringService, MaskOperations
...
def neuron_level_pruning_demo(model, layer_idx=0):
    ...
    print(f"  ✓ Can prune ANY subset of neurons (not constrained to heads)")
...
def compare_approaches(neuron_mask, head_mask, num_heads=32, head_dim=128):
    ...
    print(f"  Flexibility: Can prune ANY neurons")
    print(f"  Granularity: Individual neurons")
    ...
    print(f"  Granularity: Entire heads")
    
    print(f"\nKey Differences:")
    print(f"  ✓ Neuron-level: More flexible (any %, any neurons)")
    print(f"  ✓ Head-level: Cleaner (maintains head structure)")
    print(f"  ✓ Both supported by your framework!")
