...
def load_llama_model(model_name: str = 'gpt2', use_small_model: bool = True):
    ...
    if use_small_model:
        ...
        print(f"Using GPT-2 as demo (same architecture principles as LLaMA)")
...
def analyze_ffn_layer(
    ...
    # Capture activations
    capture = ActivationCaptureService(wrapper_ffn)
    ...
    print(f"    - RQ (alignment with input PCs)...")
    ...
    print(f"    - Redundancy (overlap with other neurons)...")
...
def main():
    ...
    print(f"  Composite = log(RQ) - 0.5*Redundancy")
