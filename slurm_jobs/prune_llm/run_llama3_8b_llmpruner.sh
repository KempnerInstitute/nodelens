#!/bin/bash
#SBATCH --job-name=paper_llama3_llmpruner
#SBATCH --output=logs/paper_llama3_llmpruner_%j.out
#SBATCH --error=logs/paper_llama3_llmpruner_%j.err
#SBATCH --nodes=1
...
# ============================================================================
# LLaMA-3.1-8B PAPER BASELINE: LLM-Pruner (Channel Mode)
# ============================================================================
# LLM-Pruner uses Taylor-based importance estimation for structured pruning.
# This is the channel-mode variant for FFN structured pruning.
# Reference: Ma et al. 2023 - "LLM-Pruner: On the Structural Pruning of LLMs"
# ============================================================================

set -euo pipefail

echo "============================================================================"
echo "SCAR Paper Baseline: LLM-Pruner | LLaMA-3.1-8B (4xGPU)"
echo "============================================================================"
...
# Run LLM-Pruner structured pruning (Taylor-based channel importance)
python scripts/run_experiment.py \
  --config configs/prune_llm/llama3_8b_full.yaml \
  --device cuda \
  --base-output-dir "$OUTPUT_BASE" \
  name="llama3_8b_paper_results_llmpruner" \
  generate_plots=true \
  pruning_strategies="['llm_pruner']" \
  pruning_amounts="[0.5]" \
...
echo ""
echo "============================================================================"
echo "LLM-Pruner baseline completed at $(date)"
echo "============================================================================"
